...
async function sendServiceSelection(chatId) {
  ...
  if (services.length === 0) {
    return await sendMainMenu(chatId, "?? **No active services available at the moment!**");
  }
...
async function sendCountrySelection(chatId, serviceId) {
  ...
  if (countries.length === 0) {
    return await sendMainMenu(chatId, `?? **No Countries Available Yet!**`);
  }
  ...
  keyboard.push([
    { text: "?? Back to Services", style: "primary" },
...
async function sendAdminPanel(chatId, messageId = null) {
  ...
  let stockText = `?? **BRO'S BOT ADMIN CONTROL PANEL** ??\n\n`;
  ...
  if (trafficData.items.length === 0) {
    stockText += `?? _No OTP traffic recorded yet._\n`;
  } else {
  ...
  if (inStockItems.length === 0) {
    stockText += `?? _No stock numbers currently available in bot database._\n`;
  } else {
      ...
      if (update.callback_query) {
        ...
        if (data === 'back_to_services') {
          ...
          const text = `?? **HOW TO UPLOAD / ADD NUMBER STOCK**\n\n` +
            ...
            `1?? Create or open a \`.txt\` file containing phone numbers (1 number per line).\n` +
            `2?? Send / Upload the \`.txt\` file directly in this Telegram chat.\n` +
            `3?? In the **File Caption**, write: \`<service> <country_code>\`\n\n` +
            `*Caption Examples:*\n` +
          ...
          if (trafficData.items.length === 0) {
            text += `?? _No OTP traffic recorded yet._\n`;
          } else {
          ...
          if (summary.length === 0) {
            text += `?? _No stock numbers or countries added yet._\n`;
          } else {
          ...
          await sendTelegramRequest('editMessageText', {
            ...
            reply_markup: {
              inline_keyboard: [
                [{ text: "?? Yes, Clear All Stock", callback_data: "admin_confirm_clearstock" }],
                [{ text: "?? Back to Admin", callback_data: "admin_stock" }, { text: "?? Main Menu", callback_data: "back_to_main_menu" }]
              ]
            }
          });
    ...
    // Handle Text Messages & File Uploads
      if (update.message) {
        ...
        if (text.startsWith('/receivesms') && isUserAdmin) {
          ...
          const firstSpace = raw.indexOf(' ');
          if (firstSpace === -1) {
            await sendTelegramRequest('sendMessage', { chat_id: chatId, text: "?? Usage: `/receivesms <number> <full message body>`\nExample: `/receivesms +255710962660 <#> 19926 es tu codigo de Facebook H29Q+Fsn4Sr`" });
            return res.status(200).json({ ok: true });
          }
        ...
        if (text.startsWith('/maintenance') && isUserAdmin) {
          ...
          await sendTelegramRequest('sendMessage', { chat_id: chatId, text: `??? Bot Maintenance mode: **${status ? 'ON ??' : 'OFF ??'}**` });
          return res.status(200).json({ ok: true });
        }
        ...
        if ((text.startsWith('/delcountry') || text.startsWith('/deletecountry')) && isUserAdmin) {
          ...
          if (code) {
            ...
            await sendTelegramRequest('sendMessage', { chat_id: chatId, text: deleted ? `? Country **${deleted.name}** (\`${deleted.code}\`) removed!` : `?? Country \`${code}\` not found!`, parse_mode: 'Markdown' });
          }
          return res.status(200).json({ ok: true });
        }
        ...
        if (text === '/admin' || text === '?? Admin Panel') {
          ...
          if (text === '?? Stock Breakdown') {
            ...
            if (inStock.length === 0) {
              msg += `?? _No stock numbers currently available in bot database._\n`;
            } else {
          ...
          if (text === '? Add Service') {
            ...
            await sendTelegramRequest('sendMessage', {
              chat_id: chatId,
              text: `? **ADD NEW SERVICE**\n\nPlease type the Service Name and Icon (e.g. \`Telegram ??\` or \`TikTok ??\` or \`Binance ??\`):`,
              parse_mode: 'Markdown',
              reply_markup: { keyboard: [[{ text: "?? Main Menu", style: "danger" }]], resize_keyboard: true, is_persistent: true }
            });
            return res.status(200).json({ ok: true });
          ...
          if (adminState[chatId]?.step === 'WAITING_DELETE_COUNTRY_CONFIRM' && text === '?? Yes, Delete Country') {
            ...
            await sendTelegramRequest('sendMessage', {
              chat_id: chatId,
              text: deleted ? `? Country **${deleted.name}** (\`${deleted.code}\`) deleted successfully!` : `?? Country deleted!`,
              parse_mode: 'Markdown'
            });
            return res.status(200).json({ ok: true });
          }
          ...
          if (adminState[chatId]?.step === 'WAITING_CLEAR_STOCK_CONFIRM' && text === '?? Yes, Clear ALL Stock') {
            delete adminState[chatId];
            clearAllStock();
            await sendAdminPanel(chatId);

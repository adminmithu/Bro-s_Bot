...
export function extractOtpCode(fullMessage) {
...
export function getFlagEmoji(countryInput) {
...
export function getServiceIcon(serviceNameOrId) {
...
export function getServices(includeDisabled = false) {
...
export function addService(id, name, icon) {
...
export function addCountry(name, code = '') {
  const cleanName = name.trim();
  const countryCode = (code || cleanName).substring(0, 3).toUpperCase();
  const flag = getFlagEmoji(countryCode) !== '??' ? getFlagEmoji(countryCode) : getFlagEmoji(cleanName);

  const existing = dbStore.countries.find(c => c.code === countryCode || c.name.toLowerCase() === cleanName.toLowerCase());
  if (existing) {
    return existing;
  }
...
export function deleteCountry(countryCode) {
...
export function addStock(serviceId, countryCode, numberList) {
...
export function getStockCount(serviceId, countryCode) {
  const key = `${serviceId.toLowerCase()}:${countryCode.toUpperCase()}`;
  return dbStore.stock[key] ? dbStore.stock[key].length : 0;
}

export function getAllStockSummary() {
  const summary = [];
...
export function exportStock(serviceId, countryCode) {
  const key = `${serviceId.toLowerCase()}:${countryCode.toUpperCase()}`;
  return dbStore.stock[key] || [];
}

export function get4Numbers(serviceId, countryCode, userId) {
  const key = `${serviceId.toLowerCase()}:${countryCode.toUpperCase()}`;
  const available = dbStore.stock[key] || [];

  if (available.length === 0) {
    return { success: false, reason: "OUT_OF_STOCK", numbers: [] };
  }
...
export function searchOTPByNumber(phoneNumber) {
...
export function parseIVASSMS(phoneNumber, fullMessage, sidInput = '', countryInput = '') {
...
export function buildOTPFormattedCard(serviceId, countryCode, phoneNumber, fullMessage, otpCode) {
  ...
  const inlineKeyboard = [
    [
      ...
      { text: "Get number ??", url: `https://t.me/${botUsername}?start=getnum`, style: "primary" }
    ]
  ];
...
export const processIncomingSMS = parseIVASSMS;

export function getUserOtpCount(userId) {
  const strId = String(userId);
...
export function clearStock(serviceId, countryCode) {
  const key = `${serviceId.toLowerCase()}:${countryCode.toUpperCase()}`;
...
export function getLiveTrafficAnalytics() {
  const analytics = {};
...
export function recordLiveRange(rangeName, phoneNumber, sid, fullMessage) {
...
export function getLiveRanges() {
  if (!dbStore.liveRanges || dbStore.liveRanges.length === 0) {
    ...
    return [
      ...
      { rangeName: 'TAJIKISTAN 16399', country: 'TAJIKISTAN', flag: '????', phoneNumber: '992889554262', sid: 'FACEBOOK', message: 'XXX XXX — ??? ??? Instagram.', time: '14:25:23' },
